...
const golfItems: GolfHubItem[] = [
  {
    label: "Round History",
    description:
      "Scorecards, drafts, trends and your last few rounds.",
    href: "/golf",
    icon: ClipboardList,
    tone:
      "bg-emerald-400/15 text-emerald-100 border-emerald-200/20",
  },
  {
    label: "Log Practice",
    description:
  ...
  },
  {
    label: "Competitions",
    description:
      "Upcoming events, prep notes and target scores.",
  ...
  },
  {
    ...
    icon: History,
    tone:
      "bg-teal-400/15 text-teal-100 border-teal-200/20",
  },
  {
    label: "Practice Plan",
    description:
    ...
    icon: CalendarDays,
    tone:
      "bg-blue-400/15 text-blue-100 border-blue-200/20",
  },
];
...
export default function GolfHub() {
  ...
  useEffect(() => {
    ...
    async function enrich() {
      const ids = unfinishedRounds.map((round) => round.id);
      ...
      unfinishedRounds.forEach((round) => {
        const rHoles = ((holeRows as RoundHole[]) || [])
          .filter(
            (hole) => hole.round_id === round.id
          );


        const rPlayers = ((players as RoundPlayer[]) || [])
          .filter(
            (player) => player.round_id === round.id
          );
        ...
        const rGames = ((games as RoundGame[]) || [])
          .filter(
            (game) => game.round_id === round.id
          );


        const scored = rHoles.filter(
        ...
        const playerScores = rPlayers.map(
          (player) => {
            const scores = rPlayerHoles.filter(
              (hole) =>
                hole.round_player_id === player.id &&
                    ...
                    unfinishedRounds.map((round) => {
                      ...
                      return (
                        <button
                          ...
                          className="flex w-full flex-col gap-3 rounded-2xl border border-line bg-white/70 px-4 py-3 text-left transition active:scale-[0.99] dark:border-emerald-200/12 dark:bg-[#0f342c]"
                        >
                          ...
                          {summary && (
                            <div className="grid grid-cols-3 gap-2 text-xs">
                              <div>
                                Hole {summary.currentHole ?? "-"}
                              </div>

                              <div>
                                {summary.totalStrokes ?? "-"} strokes
                              </div>

                              <div>
                                {summary.holesScored} scored
                              </div>
                            </div>
                          )}
                        </button>
        ...
        <section className="grid gap-3 md:grid-cols-2">
          {golfItems.map((item) => {
            ...
            return (
              <Card
                key={item.label}
                className="cursor-pointer border-line bg-white/80 dark:bg-[#0b2a24]"
                onClick={() => navigate(item.href)}
              >
                <div className="flex gap-3">
                  <span className={`rounded-2xl border p-3 ${item.tone}`}>
...
function getRoundStatusLabel(round: Round) {
  return isCompleteScoringRound(round)
    ? "Complete"
    : "In progress";
}
